...
def GeneratePlan():
	...
	passto = GenerateEnvironmentalKnowledge("passto")
        clingo_command = Command("gringo -c maxstep=35 distances.lua initialenv show.asp "+domainFile+" "+initialFile+" "+queryFile+" | ./clasp -t 6")
...
#task finished, send out "finish" action
	finish = segbot_logical_translator_gui.msg.ClingoFluent("finish",[])
	fsensedfluent =  segbot_logical_translator_gui.msg.ClingoFluent()
	fevalfluent = []
	
	goalfinished = segbot_logical_translator_gui.msg.ClingoInterfaceGoal(finish,fsensedfluent, fevalfluent)
...
#	evaluate_fluent = segbot_logical_translator_gui.msg.ClingoFluent()
	...
	for i in range(len(plan)):
		...
		if s[1][0] == "sense":
			...
			command = segbot_logical_translator_gui.msg.ClingoFluent(s[1][0], [s[1][1]])
